...
def test_full_pipeline_returns_second_by_second_mission_timeline():
    result = analyze_vehicle(
        masses=(
            MassPoint('nose', 0.100, 0.1114),
            MassPoint('fins', 0.020, 0.8222),
    ...
    assert result.mission_timeline[0].phase == 'BOOST'
    assert result.mission_timeline[-1].phase == 'LANDED'
    assert result.mission_timeline[-1].altitude_m == 0
    assert len(result.mission_timeline) > 10
