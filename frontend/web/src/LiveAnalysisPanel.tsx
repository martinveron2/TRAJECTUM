...
export function LiveAnalysisPanel({ vehicle, runToken = 0, lang = 'es' }: { vehicle: VehicleLike; runToken?: number; lang?: 'es' | 'en' }) {
  const isEs = lang === 'es';
  const txt = (es: string, en: string) => isEs ? es : en;
  const displayName = (row: ComponentRow) => {
    if (!isEs) return row.name;
    return ({ nose: 'Cofia', body: 'Fuselaje principal', motor: 'Motor', parachute: 'Paracaídas', payload: 'Carga útil', fins: 'Aletas · 4 total' } as Record<string, string>)[row.kind] ?? row.name;
  };
  ...
  const displayNote = (row: ComponentRow) => {
    ...
    const notes: Record<string, string> = {
      nose: 'xCG obtenido de la geometría de la cofia', body: 'xCG obtenido de la envolvente axial del fuselaje',
      motor: 'xCG obtenido de la envolvente axial del motor', parachute: 'envolvente de referencia del paracaídas',
      payload: 'envolvente de referencia de la carga útil', fins: 'xCG obtenido de la planta de aletas cuando está completa',
    };
    return notes[row.kind] ?? row.note;
  };
  ...
  return <div className="panel mass-panel" id="engineering-analysis">
    <div className="panel-title compact"><div><p>{txt('PROPIEDADES DE MASA DERIVADAS DE LA GEOMETRÍA', 'GEOMETRY-DERIVED MASS PROPERTIES')}</p><h2>{txt('CG de componentes → CG del vehículo → CP → vuelo → recuperación', 'Component CG → vehicle CG → CP → flight → recovery')}</h2></div>
      <button className="run" disabled={!planformReady || !componentResult || derivedMasses.length !== rows.length || running} onClick={run}>{running ? txt('EJECUTANDO…', 'RUNNING…') : planformReady ? (vehicle.cd !== '' ? txt('EJECUTAR ANÁLISIS COMPLETO', 'RUN FULL ANALYSIS') : txt('EJECUTAR CG + CP', 'RUN CG + CP')) : txt('INGRESAR GEOMETRÍA DE ALETAS', 'ENTER FIN GEOMETRY')}</button></div>
    ...
    <div className="analysis-metrics wide">
      ...
      <div><span>{txt('CG TOTAL', 'TOTAL CG')}</span><strong>{totalCg !== undefined ? `${totalCg.toFixed(1)} mm` : '—'}</strong></div>
      <div><span>CP</span><strong>{analysis ? `${analysis.cp_x_mm_from_nose.toFixed(1)} mm` : '—'}</strong></div>
      <div><span>{txt('MARGEN ESTÁTICO', 'STATIC MARGIN')}</span><strong>{analysis ? `${analysis.static_margin_calibers.toFixed(2)} cal` : '—'}</strong></div>
    ...
    {analysis?.mission_timeline && analysis.mission_timeline.length > 1 && <FlightVisualizer samples={analysis.mission_timeline} lang={lang} />}
    ...
    <div className="mass-head derived"><span>{txt('Componente', 'Component')}</span><span>{txt('Masa [g]', 'Mass [g]')}</span><span>xCG AUTO [mm]</span></div>
    <div className="mass-table">{rows.map((row) => {
      ...
      return <div className="mass-row-wrap" key={row.id}><div className="mass-row derived">
        ...
        <output>{computed ? computed.x_cg_mm.toFixed(1) : txt('POR DEFINIR', 'TBD')}</output>
