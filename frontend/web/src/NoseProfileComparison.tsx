...
export function NoseProfileComparison({ selected, lang = 'es' }: { selected: string; lang?: 'es' | 'en' }) {
  const isEs = lang === 'es';
  const txt = (es: string, en: string) => isEs ? es : en;
  return <div className="panel nose-compare">
    <div className="panel-title compact"><div><p>{txt('ESTUDIO GEOMÉTRICO', 'GEOMETRY STUDY')}</p><h2>{txt('Comparación de perfiles de cofia · mismos 180 mm × Ø63 mm', 'Nose profile comparison · same 180 mm × Ø63 mm')}</h2></div><span className="scale-note">{txt('solo geometría', 'geometry only')}</span></div>
    <div className="nose-compare-grid">{cards.map((card) => <div key={card.key} className={selected === card.key ? 'nose-card selected' : 'nose-card'}>
      ...
      <strong>{card.key === 'tangent_ogive' ? txt('Ojiva tangente', 'Tangent ogive') : card.key === 'von_karman' ? 'Von Kármán' : txt('Serie de potencias', 'Power series')}</strong><span>{card.key === 'tangent_ogive' ? txt('Referencia clásica · unión tangente suave', 'Classic reference · smooth tangent junction') : card.key === 'von_karman' ? txt('Haack C=0 · familia de baja resistencia de onda', 'Haack C=0 · low-wave-drag family') : txt('n=0.75 · alternativa geométrica más aguda', 'n=0.75 · sharper geometric alternative')}</span>
    </div>)}</div>
